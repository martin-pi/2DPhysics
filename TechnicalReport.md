# Technical Report
John Hewitt, Patrick Martin

## Abstract
Our project aims to create a 2D Java Physics engine that is capable of outperforming JBox2D in terms of object scale and ease of use. We wanted to create a framework that can be easily adapted by others for their own needs such as games. We utilize the MVC pattern so that users are able to attach their own controller and/or view to our model for their own purposes. We built our own Vector2D library so that we are able to later implement GPU acceleration, but this also means that our model is also self contained meaning it requires no dependencies or external libraries to run making it extremely easy to use in one’s own project. 

We were not able to implement a wide range of features as far as the physics goes, but we have a framework in place that can be easily expanded upon by us and other users in the future. This physics engine in its current state is able to manage an environment full of physics objects and efficiently apply various forces such as gravity, flat earth approximation gravity, and collision forces as well as generic forces to allow users to create their own customized forces. We also have the components in place to expand the models capabilities such as materials and the ability to attach multiple views to the same model. 
...
### Development
To develop our project we first started out with an outline of the general design as seen in the uml diagram below.
After we started building the MVC framework we made changes to the design based on the various problems that arose during the initial building. Eventually we ended up with a design layout detailing the current and future state of the project pictured below.

### Testing
For the most part our testing approach was simple. We created J-unit tests for each mathematical function in our Vector library and entity that simply made sure the answers we got back from each operation matched the expected answer. Testing the controller was much more difficult. We had to keep track of the time between ticks and send the number of ticks between each step to ensure that the controller was running at 60 fps. 
...
Our early estimates were roughly accurate. We ended up only missing a few of our basic, required features. Most of these features became infeasible due to our requirement of integrating them into the viewing window. This was mainly due to restrictions resulting from using OpenGL to render the viewing window. As powerful and efficient as OpenGL is, it also requires a lot of setup to get started.
